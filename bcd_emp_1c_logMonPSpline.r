...
if(user == "stefan"){
    if(Sys.info()[1]=="Darwin") {
        ...
        setwd("C:\\Users\\stefan.rameseder\\Desktop\\Useful\\Stefans Dateien\\BiddingCurves")
    }
}
...
for(pricetype in PriceTypes){ # pricetype <- PriceTypes[1]
	...
	for(pm in PM){
	  for(htnt in HTNT){ # pmhtnt <- "POS_HT"
		...
		# For testing if the algorithm works with the loaded packages:
		# test <- monotSpline(bidcurves[[ pmhtnt ]][[304]], 	deriv = 0, lambda_opt = TRUE, increasing = TRUE, trace = FALSE)
